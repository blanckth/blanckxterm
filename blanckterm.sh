...
local -r vncCfg="$vncDir/config";
echo '#!/usr/bin/env bash' > $vncXS;
echo 'xfce4-session &' >> $vncXS;
echo "localhost" > $vncCfg;
echo "geometry=2333x1080" >> $vncCfg;
echo 'export DISPLAY=localhost:0' >> "$etcDir/bash.bashrc";
# VNC Start
vncserver -name blanckxterm;
vncserver -list;
}
###
MAIN () {
echo;
ChColors MENU;
TypingS "Welcome To BlanckTerm!  By Salar Muhammadi!" 0.0123; echo;
TypingS "Automatic Configuration Termux By Salar Muhammadi!" 0.0123; echo;
ChColors MENUQ;
TypingS "Salar Muhammadi!" 0.0123; echo;
SGR RSGR;
echo;
pingGo; echo; ! [[ "$?" -eq 0 ]] && ChColors FAILED && TypingS "Check Internet Connection and again Enter $aPp" 0.0123 && SGR RSGR && exit 111;
[[ -f $isUpg ]] && [[ `cat "$isUpg"` -eq 0 ]] && echo || fupg && [[ `cat "$isUpg"` -eq 0 ]] && echo || ChColors FAILED && TypingS "Check System Upgrade Error and again Enter $aPp" 0.0123 && SGR RSGR && exit 222;
